...
CREATE [UNIQUE|BITMAP] INDEX 인덱스명
    ON 테이블명(컬럼명][, 컬럼명2,...]) [ASC|DESC]
    
CREATE 로 만든 객체는 삭제할 때 DROP을 쓴다고 합니다.

(사용예)
상품테이블에서 상품명으로 NORMAL INDEX를 구성하시오
CREATE INDEX IDX_PROD_NAME
ON PROD(PROD_NAME);

사용예) 장바구니 테이블에서 장바구니번호 중 3번째에서 6글자로 인덱스를 구성하시오
...
- 인덱스의 재구성
 ...
 - 블록 (BLOCK) 구조로 구성
 - 미리 컴파일되어 실행 가능한 상태로 서버에 저자외어 필요시 호출되어 사용됨
 - 모듈화, 캡슐화 기능 제공
 - Annonymous Block, Stored Procedure, User Defined Function, Package, Trigger 등으로 구성
...
1) 변수, 상수 선언
    ...
    (3) 데이터타입
        ...
        - PLS_INTEGER, BINARY_INTEGER : 214783647~-214783648까지 자료처리
